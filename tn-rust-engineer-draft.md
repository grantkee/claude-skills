...
At epoch boundaries the on-chain committee contract is read, a new committee is formed, and
all epoch-scoped resources (TaskManager, ConsensusBus epoch channels, storage) are recycled.

Non-validator "observer" nodes follow consensus headers via gossipsub and execute blocks
without participating in DAG construction.

## Workspace Architecture

Root: `Cargo.toml` workspace with `resolver = "2"`, edition 2021, MSRV 1.94.

### Crate layers (dependency flows top-to-bottom)

```
bin/telcoin-network          -- binary entry point
crates/telcoin-network-cli   -- CLI commands (clap)
crates/node                  -- NodeManager orchestrates epochs, owns TaskManagers
crates/engine                -- consensus-to-execution bridge, owns Reth pipeline
crates/consensus/primary     -- DAG primary (proposer, certifier, bullshark, ConsensusBus)
crates/consensus/worker      -- batch builder sealing + worker network
crates/consensus/executor    -- subscriber that orders committed subdags
crates/batch-builder         -- transaction batch construction from pool
crates/batch-validator       -- batch validation for non-authors
crates/execution/tn-rpc      -- TN-specific RPC extensions
crates/tn-reth               -- Reth integration (chainspec, EVM config, pipeline)
crates/state-sync            -- epoch/consensus state sync for lagging nodes
crates/network-libp2p        -- libp2p networking (gossipsub, req-res, kademlia, streams)
crates/network-types         -- shared network type definitions
crates/storage               -- consensus DB (tables macro, redb/mdbx backends)
crates/config                -- node configuration types
crates/types                 -- core types, crypto, codec, TaskManager, Notifier, sync traits
crates/tn-utils              -- misc utilities
crates/test-utils            -- integration test helpers (CommitteeFixture re-export, etc.)
crates/test-utils-committee  -- CommitteeFixture builder for test committees
crates/e2e-tests             -- end-to-end integration tests
```

### Dependency rules

- `tn-types` is the foundation -- almost everything depends on it
- `tn-storage` depends on `tn-types` only
- Consensus crates depend on `tn-types`, `tn-storage`, `tn-config`, `tn-network-libp2p`
- Execution crates (`tn-reth`, `batch-builder`, `batch-validator`) depend on `tn-types` and reth
- `tn-engine` bridges consensus and execution
- `tn-node` orchestrates everything -- depends on engine, consensus, networking
- **Never** introduce upward dependencies (e.g., `tn-types` must not depend on `tn-storage`)
